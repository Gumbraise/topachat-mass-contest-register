...
def main(input_path, output_path):
    ...
    if os.path.isdir(input_path):
        ...
        return run_tesseract(filename, output_path, input_path)
...
def mainogur():
    ...
    email1 = ''.join(random.choice(letters) for i in range(10))
    email2 = email1+'@kellis.fr'
    ...
    yoo.clear()
    yoo.send_keys(email2)
    time.sleep(.5)
    yoo2 = driver.find_element_by_xpath('//form[@id="concours"]/p[2]/input[2]')
    yoo2.clear()
    yoo2.send_keys(email2)
    time.sleep(.5)
    yoo3 = driver.find_element_by_xpath('//div[@id="captcha"]/img').get_attribute("src")
    filename ='grrr.png'

    downloader(yoo3, filename)
    captcha = main(filename, None)
    captcha = captcha[:-2]

    yoo2 = driver.find_element_by_xpath('//input[@name="captcha-verif"]')
    yoo2.clear()
    yoo2.send_keys(captcha)
    time.sleep(.5)
    yoo2 = driver.find_element_by_xpath('//input[@value="Jouer !"]')
    yoo2.click()
